...
const CONFIG = {
  ...
  quiz: 'auto',        // 'off' | 'auto' | 'choice' | 'typing' — 'choice' 는 객관식만: 오답 3개가 안 되는 카드는 출제에서 빠진다
...
| `'choice'` | 객관식만. 오답 3개를 못 만드는 basic은 주관식으로 폴백하지 않고 **출제에서 빠진다** — 주관식이 섞이면 안 되는 덱(수업용 4지선다)에만 쓴다 |
| `'typing'` | 철자·정확한 표기가 목표 (영단어, 명령어, API 이름) — basic도 주관식으로 |
| `'off'` | 카드 학습만. 퀴즈 탭이 숨는다 |

객관식은 4지선다라 **오답 보기 3개**가 있어야 성립한다(`choices` + 다른 카드의 back). `quiz: 'auto'`에서
못 채우면 그 카드는 자동으로 주관식으로 폴백하고, `'choice'`에서는 빠진다. 답이 코드블록·이미지뿐인
카드와 sequence·mermaid 카드는 어느 모드에서든 출제에서 제외된다. 출제 가능한 카드가 0장이면 퀴즈 탭
자체가 숨는다 — 덱이 작으면(basic 4장 미만) 퀴즈가 객관식이 아닐 수 있음을 사용자에게 알린다.
...
const DECK = [
  { id: 1,                     // 고유 번호(필수). localStorage 상태 매칭용 — 재사용/중복 금지
    ...
    choices: ['오답1','오답2','오답3'], // 선택. 객관식 오답을 직접 지정 — 3개면 덱이 작아도 4지선다가 성립한다. 모자란 만큼은 다른 카드 back에서 채운다
  },
  { id: 2, type: 'cloze',
...
| 키보드 | Space/Enter, ←→, 1, 2, S, P / 발표 중 Space·Enter·→·PageDown(공개·다음)·←·PageUp(뒤로)·A·Esc / 확대 중 0·Esc(원래 크기). 확대 → 발표 순서로 핸들러 최상단에서 검사 |
